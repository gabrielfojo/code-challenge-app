    ...
    function render(el){
        ...
        $( "#sortable" ).sortable({
            ...
            stop: function( event, ui ) {
                ...
                axios.put(APIURL+'/api/items/order',changeOrder)
                .then(console.log)
                .catch(console.log);
...
$( document ).on( "submit", "form", function( event ){
    ...
    if(state.modal_id){
        // Update
        axios.put(APIURL+'/api/items/'+state.modal_id, formData)
        .then(_=>{
            app.init(myApp);
        })
        .catch(console.log);
...
//== Delete item
$( document ).on( "click", "button[data-type='btn_delete']", function( event, data ){
...
$( document ).on( "input", "#myModal_description", _.debounce((function(){
...
// Local image getter
 function readURL(input) {  
    ...
    if (input.files && input.files[0]) {
        if(!fileTypes.includes(input.files[0].type) ){
            $('#myModal_error').html(Alert({msg:'<strong>Try again..</strong> ( jpg | gif | png )'}));
            $('#myModal_save').attr('disabled','disabled');
            return false;
        }
        ...
        reader.onload = function(e) {
            ...
            image.onload = function () {
                ...
                if(this.height == 320 && this.width===320){
                    ...
                    $('#myModal_error').html(Alert({msg:'<strong>Try again..</strong> ( 320 x 320 )'}));
                    $('#myModal_save').attr('disabled','disabled');
                    return false;
                }
            }
 ...
 // Reactivity 
 $( document ).on( "change", "input[data-model],textarea[data-model]", function( event, data ){;
